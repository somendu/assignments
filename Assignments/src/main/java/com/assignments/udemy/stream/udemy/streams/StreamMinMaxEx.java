...
public class StreamMinMaxEx {
	...
	public static int findMaxValue(List<Student> noteBookCountList) {
		...
		int maxNoteBookCount = noteBookCountList.stream().map(student -> student.getNoteBooks())
	...
	public static Optional<Integer> findMaxvalueOptional(List<Integer> intList) {

		return intList.stream().reduce((a, b) -> a > b ? a : b);
	}

	public static Optional<Integer> findMinvalueOptional(List<Integer> intList) {
	...
	public static void main(String[] args) {
		...
		if (findMaxvalueOptional(intList).isPresent()) {
			...
			System.out.println("Max Value is : " + findMaxvalueOptional(intList).get());
		} else {
		...
		if (findMinvalueOptional(intList).isPresent()) {
			...
			System.out.println("Min Value is : " + findMinvalueOptional(intList).get());
		} else {
			System.out.println("List is empty");
		}
